...
import { sendMessage } from "~/models/main.server";
...
export async function action({ request }: ActionFunctionArgs) {
    ...
    if (action === "send-message") {
        ...
        if (typeof name != "string" || name.length <= 3) {
            return json({ status: 418, message: "Invalid name" })
        }
        if (typeof phone != "string" || phone.length <= 3) {
            return json({ status: 418, message: "Invalid phone" })
        }
        if (typeof message != "string" || message.length <= 3) {
            return json({ status: 418, message: "Invalid message" })
        }

        const savedMessage = await sendMessage({ name, phone, message })

        return json({ status: 200, message: `Message Sent - ${new Date(savedMessage.createdAt).toDateString()}` })
    }

    return json({ status: 418, message: "oops" })
};
...
export default function ContactPage() {
    ...
    return (
        <div className="min-h-screen w-full bg-cover bg-center bg-[url('https://images.pexels.com/photos/3244513/pexels-photo-3244513.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1')]">
            <SideBar />
            <div className="text-center flex self-center justify-center mx-auto w-full">
            ...
            <div
                className="mx-auto mt-24 px-8 w-full max-w-4xl flex flex-col lg:flex-row md:px-12 lg:px-16 xl:px-32 py-16 bg-gray-100 bg-opacity-90 text-gray-900 rounded-lg shadow-lg">
                <div className="flex flex-col justify-between px-4 text-center mx-auto w-full">
                    <div>
                ...
                <Form method="post" replace className="w-full">
                    <input type="hidden" name="_action" value="send-message" />
                    <div>
                        <span className="uppercase text-sm text-gray-600 font-bold">Name</span>
                        <input name="name" value={name} onChange={(e) => setName(e.target.value)} required className="w-full bg-gray-300 border border-gray-400 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
                            type="text" placeholder="Enter Name" />
                    </div>
                    <div className="mt-8">
                        <span className="uppercase text-sm text-gray-600 font-bold">Phone</span>
                        <input name="phone" value={phone} onChange={(e) => setPhone(e.target.value)} required className="w-full bg-gray-300 border border-gray-400 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
                    ...
                    <div className="mt-8">
                        ...
                        <textarea
                            ...
                            className="w-full h-32 bg-gray-300 border border-gray-400 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"></textarea>
                    </div>
                    <div className="mt-8">
                        <button
                            type="submit"
                            className={`${actionData?.status === 200 ? "bg-green-500" : "bg-indigo-500"} uppercase text-sm font-bold tracking-wide text-gray-100 p-3 rounded-lg w-full focus:outline-none focus:shadow-outline`}>
                            {actionData?.status === 200 ? "Message Sent" : "Send Message"}
                        </button>
                    </div>
                </Form>
            </div>
